...
- 그다음 웹 UI에서 "오디오 파일을 올리고, 재생하고, 기본 session 상태를 확인하는 흐름"을 MVP로 만든다.
- MVP 이후에는 같은 command 경계를 단축키와 자동화 surface로 차례로 노출한 뒤, 작업물을 잃지 않는 기능, 편집 속도를 높이는 기능, 더 복잡한 제작 기능 순서로 확장한다.
- 내부 구조 기준이 아니라 사용자 가치 기준으로 phase를 나눈다.
- 기능 구현 순서는 `TDD 테스트 작성 -> command/domain 로직 구현 -> CLI로 동작 검증 -> UI 연결`을 기본으로 한다.
- 초기 phase에서는 웹 UI, 단축키, CLI가 같은 command 실행 경계를 공유하게 만든다.
...
- MVP 완료 기준은 Phase 1 종료 시점이다. Phase 0은 MVP가 아니라 웹 작업 화면이 사용할 command 실행 경계를 준비하는 단계다.

## Phase 0. CLI command 기반 구축

웹 작업 화면을 붙이기 전에 CLI에서 command 실행 경계를 검증한다. 여기서 CLI는 문자열 입력을 command로 parse하고 `AppController.executeCommand()`로 실행하는 조작 surface를 뜻한다.

이 phase는 MVP 완료가 아니다. Phase 1의 웹 작업 화면이 재사용할 command registry, session read model, 실행 결과 형식을 먼저 고정하는 선행 단계다.
...
## Phase 1. MVP: 웹 UI 붙이기

사용자는 Phase 0 workflow를 브라우저 화면에서 확인하고 조작할 수 있다.

여기서 웹 UI는 새 편집 상태 소유자가 아니라, 같은 command 실행 경계와 session read model을 사용하는 브라우저 control surface를 뜻한다.

Phase 1 종료 시점을 MVP 완료 기준으로 본다. 이때 MVP는 "사용자가 웹에서 오디오 파일을 업로드하고, session 상태를 확인하고, 재생 조작과 CLI command 실행을 할 수 있는 상태"를 뜻한다.

체크 기준: 현재 브라우저 화면에서 직접 확인 가능한 항목만 체크한다. controller/domain에만 존재하거나 CLI에서만 가능한 항목은 체크하지 않는다.
...
## Phase 7. 기본 이펙트와 Processor Chain
...
## Phase 11. Automation, Tempo, Arrangement
...
여기서 script는 여러 command를 저장된 절차로 묶어 반복 실행하는 자동화 기능이다. 예를 들어 "선택 구간을 export하고 파일 이름 규칙을 적용한다" 같은 반복 작업을 사용자가 저장해두고 다시 실행하는 기능을 뜻한다.
