...
- [x] 자연어 요청 입력
  - `AgentPanel`이 사용자-facing request textarea를 제공한다.
  - 빈 request는 planning을 시작하지 않고 error message를 표시한다.
- [x] command 후보 생성
  - `IAgentPlanner` 인터페이스와 deterministic `ScriptedAgentPlanner`는 있다.
  - provider-agnostic `HttpAgentPlanner` adapter는 있다.
  - `WebLLMAgentPlanner`가 WebLLM chat completion 응답을 `AgentPlanDraft` 후보로 변환한다.
  - `createDefaultAgentPlanner()`는 설정이 없으면 기존 scripted planner를 사용한다.
...
- [x] WebLLM planner 연결
  ...
  - WebLLM structured output schema는 empty `steps`를 허용해, 매칭되는 command가 없을 때 `playback.play` fallback을 강제하지 않는다. empty plan은 기존 plan validation에서 실패 처리한다.
