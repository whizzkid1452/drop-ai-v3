...
- [ ] 자연어 요청 입력
  - `requestPlan({ requestText })` API는 있다.
  - 사용자가 입력할 AgentPanel 또는 동등한 사용자-facing 입력 surface는 아직 없다.
- [ ] command 후보 생성
  - `IAgentPlanner` 인터페이스와 deterministic test planner는 있다.
  - 실제 자연어 기반 planner adapter는 아직 없다.
- [ ] command plan preview
  - 검증된 `AgentCommandPlan` draft를 반환하는 core 흐름은 있다.
  - 사용자가 plan 내용을 확인하는 preview UI는 아직 없다.
- [ ] command plan 사용자 승인
  - `approvePlan()` / `rejectPlan()` API는 있다.
  - 사용자가 승인 또는 거절하는 UI는 아직 없다.
- [x] 승인된 command 실행
...
- [ ] command 실행 실패 메시지 표시
  - 실패 code/message를 execution result와 audit entry로 반환하는 core 흐름은 있다.
  - 사용자-facing 실패 메시지 표시는 아직 없다.
