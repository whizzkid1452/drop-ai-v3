...
체크 기준: 현재 `main` 기준으로, `[x]`는 MVP 의미까지 구현된 항목이다. 내부 API 또는 테스트용 구현만 있는 항목은
체크하지 않고, 현재 완료된 범위를 하위 항목에 적는다.

- [x] agent command workflow
  - `AgentWorkflow`가 plan 요청, plan 검증, reject/approve, stale plan 차단, 승인된 command 실행을 담당한다.
- [x] agent가 사용할 command registry 노출
  - `agentCommandCatalog`가 agent가 사용할 command metadata, payload 설명, 예시, 사용 가능 여부를 제공한다.
- [x] agent가 읽을 session summary 제공
  - `createAgentSessionSummary()`가 session 상태를 agent 입력용 read model로 변환한다.
- [x] 자연어 요청 입력
  - `AgentPanel`이 사용자-facing message textarea와 assistant message list를 제공한다.
  - 빈 message는 response 생성을 시작하지 않고 error message를 표시한다.
- [x] command 후보 생성
  - `IAgentPlanner` 인터페이스와 deterministic `ScriptedAgentPlanner`는 있다.
  - provider-agnostic `HttpAgentPlanner` adapter는 있다.
  - `WebLLMAgentPlanner`가 WebLLM chat completion 응답을 assistant message와 `AgentPlanDraft` 후보로 변환한다.
  - `createDefaultAgentPlanner()`는 설정이 없으면 WebLLM planner를 사용한다.
  - `VITE_AGENT_PLANNER_PROVIDER=scripted | http | webllm`로 Web path의 planner provider를 선택할 수 있다.
  - `http` provider는 `VITE_AGENT_PLANNER_ENDPOINT`가 필요하다. 이 endpoint 구현은 현재 repository 범위가 아니다.
- [x] WebLLM planner 연결
  - `WebLLMAgentPlanner`는 WebLLM 응답을 `AgentPlanDraft` 후보로 변환하는 `IAgentPlanner` 구현체를 뜻한다.
  - planner input은 `requestText`, `sessionSummary`, command catalog로 제한한다.
  - WebLLM planner는 command를 직접 실행하지 않고, command 실행은 계속 `AgentWorkflow`의 preview/approve 경계를 거쳐 `AppController.executeCommand()`에서만 수행한다.
  - WebLLM 응답은 신뢰하지 않고 기존 plan validation을 통과한 draft만 preview에 표시한다.
  - WebLLM structured output schema는 command type을 agent 사용 가능 catalog literal로 제한한다.
  - WebLLM structured output schema는 `command` 객체의 extra key를 차단하고, `WebLLMAgentPlanner`는 no-payload command의 빈 `payload`와 command 내부 `reason`을 validation 전에 정리한다.
  - WebLLM prompt payload는 agent가 사용할 수 있는 전체 command type 목록과 command selection rule을 포함한다.
  - WebLLM system prompt는 command selection rule과 한국어 intent 예시를 포함해 `playback.play` fallback을 줄인다.
  - WebLLM structured output schema는 empty `steps`를 허용해, 매칭되는 command가 없을 때 `playback.play` fallback을 강제하지 않는다. empty steps는 일반 assistant message로 처리한다.
  - `AgentChatWorkflow`는 request text에 명시적인 DAW action intent가 있을 때만 WebLLM의 `steps`를 draft plan 후보로 승격한다.
  - 모델 로딩 실패, planner 응답 형식 불일치, plan validation 실패는 planning failure로 표시한다.
  - WebLLM provider가 선택되면 페이지 진입 시점에 모델 preload를 시작한다.
  - `AgentPanel`은 WebLLM init progress callback이 전달한 모델 로딩 진행률을 preload 또는 planning 중 progressbar로 표시한다.
  - 실제 브라우저 실행은 WebGPU 지원, WebLLM 모델 다운로드, 선택한 모델의 JSON 응답 품질에 의존한다.
- [x] command plan preview
  - 검증된 `AgentCommandPlan` draft를 반환하는 core 흐름이 있다.
  - `AgentPanel`이 command type, step id, reason, payload를 실행 전에 표시한다.
- [x] command plan 사용자 승인
  - `approvePlan()` / `rejectPlan()` API가 있다.
  - `AgentPanel`이 draft plan의 approve/reject control을 제공한다.
- [x] 승인된 command 실행
  - 승인 시 `AppController.executeCommand()` 경계로 command를 순차 실행한다.
- [x] command 실행 결과 요약
  - `summarizeCommandResultForAudit()`가 성공 결과와 export 결과 metadata를 요약한다.
- [x] command 실행 실패 메시지 표시
  - 실패 code/message를 execution result와 audit entry로 반환하는 core 흐름이 있다.
  - `AgentPanel`이 planning failure, plan validation failure, command execution failure message를 표시한다.
  - plan validation failure 중 invalid command는 실패한 command type과 payload validation issue path를 표시한다.
- [x] structured command audit log
  - `AgentAuditLog`가 plan과 command 실행 이벤트를 structured entry로 기록한다.
